...
delete_spectra_q = input('Should a spectrum be removed from evaluation (y/n):')
if delete_spectra_q == 'y' : 
    delete_mult_spectra = delete_spectra_q
    while delete_mult_spectra == 'y':
        delete_spectra_a = int(input('Which spectrum should be removed from 1 - ' + str(spektren_df.columns.size) + ': '))
        if delete_spectra_a >=1 and delete_spectra_a <= spektren_df.columns.size :
           spektren_df.drop(spektren_df.columns[int(delete_spectra_a) - 1], axis=1, inplace= True) 
           #generate empty list to put in difference values to mean spectrum later
           diff_list = []
           #generate plot of single spectra
           for col in spektren_df.columns:
           ...
           print('The spectrum ' + str(max_diff_meas) + ' has highest difference to mean spectrum and might be an outlier.')
            ...
            print('wrong input')
        delete_mult_spectra = input('Do you want to delete further spectra? (y/n)')
...
if delete_spectra_q == 'y':
    delete_measurement = input('Do you want to delete the spectrum as well from EDX results table? (y/n):')
    if delete_measurement =='y':
        del gefundene_dateien[int(delete_spectra_a)-1]
